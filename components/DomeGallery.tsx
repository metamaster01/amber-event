  ...
  useEffect(() => {
    ...
    const startAutoRotate = () => {
      const animate = (currentTime: number) => {
        if (!lastAutoRotateTime.current) {
          lastAutoRotateTime.current = currentTime;
        }

        const deltaTime = currentTime - lastAutoRotateTime.current;
        lastAutoRotateTime.current = currentTime;

        // Don't auto-rotate if user is dragging or image is enlarged
        if (!draggingRef.current && !focusedElRef.current) {
          const rotationDelta = (autoRotateSpeed * deltaTime) / 16.67; // Normalized to 60fps
          const nextY = rotationRef.current.y + rotationDelta;
  ...
  useGesture(
    {
      onDragStart: ({ event }) => {
        if (focusedElRef.current) return;
        stopInertia();
        stopAutoRotate(); // Stop auto-rotation when user starts dragging
